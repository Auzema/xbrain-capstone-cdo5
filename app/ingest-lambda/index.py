...
def _stable_alert_id(*parts: Any) -> str:
    normalized_parts = [
...
def _is_alertmanager_payload(payload: Dict[str, Any]) -> bool:
    return isinstance(payload.get("alerts"), list)


def _alertmanager_alert_to_cdo(payload: Dict[str, Any], alert: Dict[str, Any]) -> Dict[str, Any]:
    common_labels = payload.get("commonLabels") or {}
    common_annotations = payload.get("commonAnnotations") or {}
    alert_labels = alert.get("labels") or {}
    alert_annotations = alert.get("annotations") or {}

    labels = {
        **(common_labels if isinstance(common_labels, dict) else {}),
        **(alert_labels if isinstance(alert_labels, dict) else {}),
    }
    annotations = {
        **(common_annotations if isinstance(common_annotations, dict) else {}),
        **(alert_annotations if isinstance(alert_annotations, dict) else {}),
    }
    ...
    service = _first_value(
        ...
        labels.get("label_app_kubernetes_io_name"),
        labels.get("kubernetes_name"),
        labels.get("job"),
        labels.get("deployment"),
        labels.get("container"),
    )
    started_at = alert.get("startsAt") or alert.get("starts_at")
    fingerprint = alert.get("fingerprint")
    alert_id = _first_value(
        labels.get("alert_id"),
        annotations.get("alert_id"),
        fingerprint,
        _stable_alert_id(alertname, service, started_at, labels),
    )
    ...
    cdo_payload = {
        ...
        "source": labels.get("source") or "alertmanager",
        "service": service,
    ...
    optional_aliases = {
        ...
        "metric_names": ["metric_names", "__name__", "metric"],
        "trace_id": ["trace_id", "traceID"],
        "status_code": ["status_code", "code", "http_status"],
        "reason": ["reason", "alertname"],
    ...
    for field, aliases in optional_aliases.items():
        for alias in aliases:
            value = labels.get(alias) or annotations.get(alias)
            if value is not None and value != "":
                cdo_payload[field] = value
                break
...
def _write_artifacts(
    ...
    bucket = os.environ.get("AUDIT_BUCKET_NAME", "").strip()
    if not bucket:
        raise RuntimeError("AUDIT_BUCKET_NAME is required before publishing normalized alerts")
...
def _write_rejection_artifact(
    ...
    pre_correlation_prefix = os.environ.get("S3_PREFIX_PRE_CORRELATION", "pre-correlation").strip("/")
    yyyy, mm, dd = _date_parts(received_at)
    key = f"invalid/{pre_correlation_prefix}/rejected-alerts/{yyyy}/{mm}/{dd}/{ingest_id}.json"
    document = {
        "ingest_id": ingest_id,
        "schema_version": "cdo.ingest_rejection.v1",
        "received_at": _iso_z(received_at),
        "status": "REJECTED",
    ...
    return _put_json(bucket, key, document)
...
def _process_alertmanager_payload(
    event: Dict[str, Any],
    payload: Dict[str, Any],
    received_at: datetime,
    ingest_id: str,
    ...
    if not isinstance(alerts, list) or not alerts:
        return (
            400,
            {
                "status": "REJECTED",
                "ingest_id": ingest_id,
                "reason": "alertmanager_alerts_required",
                "sqs_published": False,
            },
        )
    ...
    for index, alert in enumerate(alerts, start=1):
        item_ingest_id = f"{ingest_id}-{index:04d}"
            ...
            results.append(
                {
                    "index": index,
                    "status": "REJECTED",
                    "ingest_id": item_ingest_id,
                    "reason": "alert_item_must_be_object",
                    "sqs_published": False,
                }
            )
            continue
        ...
        cdo_payload = _alertmanager_alert_to_cdo(payload, alert)
        status_code, result = _process_alert_payload(
            event,
            cdo_payload,
            received_at,
            item_ingest_id,
        ...
        result["index"] = index
        result["alert_id"] = cdo_payload.get("alert_id")
        results.append(result)

        if result.get("sqs_published"):
            published_count += 1
    ...
    if published_count:
        ...
        status = "DUPLICATE"
        status_code = 200
    elif skipped_count and not rejected_count:
        status = "SKIPPED"
        status_code = 200
    else:
        status = "REJECTED"
...
def handler(event: Dict[str, Any], _context: Any) -> Dict[str, Any]:
    ...
    if not _verify_signature(event, body):
        _write_rejection_artifact(body, received_at, ingest_id, "invalid_signature", 401)
        return _response(401, {"status": "REJECTED", "ingest_id": ingest_id, "reason": "invalid_signature"})

    try:
        payload = json.loads(body.decode("utf-8"))
    except json.JSONDecodeError:
        _write_rejection_artifact(body, received_at, ingest_id, "invalid_json", 400)
        return _response(400, {"status": "REJECTED", "ingest_id": ingest_id, "reason": "invalid_json"})

    if not isinstance(payload, dict):
        _write_rejection_artifact(body, received_at, ingest_id, "payload_must_be_object", 400)
        return _response(400, {"status": "REJECTED", "ingest_id": ingest_id, "reason": "payload_must_be_object"})

    if _is_alertmanager_payload(payload):
        status_code, response_body = _process_alertmanager_payload(
            event,
            payload,
            received_at,
            ingest_id,
    ...
    status_code, response_body = _process_alert_payload(
        event,
        payload,
        received_at,
        ingest_id,
