...
async def alertmanager_webhook(payload: dict):
    print("Received alert from Alertmanager:", json.dumps(payload, indent=2))
    
    # Forward the alerts to platform-service
    platform_url = os.getenv("PLATFORM_SERVICE_URL", "http://platform-service:8080/api/v1/notify")
    ...
    if "alerts" in payload:
        async with httpx.AsyncClient() as client:
            for am_alert in payload["alerts"]:
                ...
                started_at = am_alert.get("startsAt", datetime.now().isoformat() + "Z")
                ...
                triage_request = {
                    ...
                    "ownership": {
                        "service": service,
                        "owner_team": "core-payments" if severity == "critical" else "platform-ops",
                        "slack_channel": "#oncall-payments",
                        "jira_project": "PAY" if severity == "critical" else "OPS",
                        "runbooks": []
                    }
                }
                
                try:
                    print(f"Forwarding to platform-service: {platform_url}...")
                    resp = await client.post(platform_url, json=triage_request, timeout=10.0)
                    print(f"Platform-service response status: {resp.status_code}")
                    print(f"Platform-service response body: {resp.text}")
                except Exception as e:
                    print(f"Failed to forward alert to platform-service: {e}")
