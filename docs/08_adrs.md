...
- **Date**: 2026-06-24
- **Context**: 
  Hệ thống cần lưu trạng thái xử lý từng incident (`CREATED`, `AI_PROCESSING`, `JIRA_CREATED`, `SLACK_NOTIFIED`, `FAILED`) và đảm bảo tính idempotency (chống trùng lặp) để tránh tạo ticket Jira hoặc gửi Slack trùng khi có cơ chế retry. Idempotency ở đầu vào (Ingest Lambda chống trùng webhook Alertmanager) và trạng thái workflow (Correlator Worker) có pattern truy cập, tần suất ghi và vòng đời dữ liệu khác nhau, nên tách thành 2 bảng để phân quyền IAM tối thiểu.
  
- **Decision**: 
  Chọn **DynamoDB On-Demand (PAY_PER_REQUEST)** với **2 bảng độc lập** (khớp `modules/storage` và sơ đồ infra):
  - **`${name_prefix}-incident-state`**: hash key `incident_id`. Hai GSI: `idempotency-key-index` (hash `idempotency_key`) và `tenant-service-index` (hash `tenant_service_key`) — phục vụ tra cứu chống trùng side-effect và audit theo tenant/service. TTL trên `expires_at`, bật PITR. Do Correlator Worker + Integration layer ghi.
  - **`${name_prefix}-idempotency`**: hash key `PK` (hash của alert gốc). TTL trên `expires_at` (~30 ngày) tự dọn khóa cũ, bật PITR, bật deletion-protection ở prod. Chỉ Ingest Lambda đọc/ghi.
...
- **Consequence**:
  ...
  - ✅ **Tự động dọn dẹp (TTL trên `expires_at`)**: Bảng `idempotency` tự xóa khóa cũ (~30 ngày) mà không cần custom cleanup cronjob; bảng `incident-state` giữ theo TTL cấu hình phục vụ audit.
...
- **Consequence**:
  ...
  - ✅ **Giảm thiểu blast radius nhờ Canary**: Deploy canary giúp giảm rủi ro cập nhật lỗi; Argo Rollouts tự động abort/rollback nếu vi phạm abort criteria: error rate > 0.5% (5xx), P99 latency > 1000ms, hoặc AI confidence trung bình < 0.5.
...
- **Decision**: 
  ...
  - Đặt thời hạn tự động dọn dẹp log (retention policy) ngắn hạn (7 ngày cho CloudWatch Logs, khớp `log_retention_days=7` trong Terraform cho cả 3 môi trường; Loki retention cấu hình tương đương).
- **Consequence**:
...
- **Context**: 
  ...
  Prometheus/Alertmanager (Webhook) ──► Ingest Lambda ──► SQS FIFO ──► AIOps Worker ──► TF1 AI Engine (compute-first RCA; Bedrock synthesis optional, AI-team bật khi cần) ──► DynamoDB & S3 ──► Jira/Slack.
