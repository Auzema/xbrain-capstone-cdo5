...
CDO-05 sử dụng kiến trúc EKS-native với **3 môi trường** (Sandbox, Staging, Production). Chi phí AWS được phân tích trực tiếp theo từng môi trường thay vì phân chia một cách khiên cưỡng theo từng tenant (khách thuê). Chi phí được chia làm hai loại chính:
...
> **Môi trường Staging (2 AZs, us-east-1):** cấu hình tương đương Sandbox (2 AZ, WAF/CloudTrail tắt), nên chi phí cố định xấp xỉ **~$342/tháng**. Staging đóng vai trò cổng pre-prod (integration/E2E test) trước khi promote lên Production, tách biệt với Sandbox dùng cho dev/experiment.
...
*   **Amazon Bedrock (Nova Micro)** — *chỉ phát sinh khi AI team bật LLM synthesis; mặc định engine chạy compute-first nên chi phí Bedrock = $0*:
...
*   ✅ **Non-prod tối giản (Sandbox & Staging dùng 2 AZ)**: Chỉ Production chạy 3 AZ (High Availability) và bật WAF/CloudTrail; Sandbox/Staging giữ 2 AZ và tắt WAF/CloudTrail, cắt bớt chi phí NAT/endpoint/WAF ở non-prod mà vẫn đủ để integration/E2E test.
...
*   ✅ **Đặt giới hạn Retention cho CloudWatch Logs**: Giới hạn thời gian lưu trữ logs trong 7 ngày (thay vì vô hạn mặc định) để tránh tích lũy dung lượng lưu trữ đắt đỏ qua thời gian.
...
*   [`04_deployment_design.md`](04_deployment_design.md) — Chiến lược triển khai 3 môi trường (Sandbox/Staging/Prod).
