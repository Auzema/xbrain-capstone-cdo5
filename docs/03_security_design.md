...
|---|---|---|---|
| `tf-1-alb-sg` | 443 từ CDO platforms trong task force | 8080 to app-sg | ALB |
| `tf-1-app-sg` | 8080 từ alb-sg | 5432 to db-sg, 443 to Bedrock/Secrets Manager | ECS/Lambda task |
| `tf-1-db-sg` | 5432 từ app-sg | (none) | RDS/DocDB |
...
|---|---|---|
| `tf-1-ai-engine-task-role` | ECS task / Lambda | `bedrock:InvokeModel`, `secretsmanager:GetSecretValue` (specific ARN), `s3:PutObject` (audit bucket only) |
| `tf-1-platform-deploy-role` | CI/CD pipeline | ECS deploy, ECR push, CloudFormation deploy. KHÔNG có `iam:*`, `s3:Delete*` |
| `tf-1-readonly-role` | Mentor review / debug | CloudWatch logs read, ECS describe, S3 GetObject |
...
|---|---|---|---|
| `BEDROCK_API_KEY` | Secrets Manager `tf-1/ai-engine/bedrock` | manual (no rotation) | ai-engine task role |
| `DB_CREDENTIALS` | Secrets Manager `tf-1/platform/db` | 30 days auto | app-sg |
| `WEBHOOK_SIGNING_KEY` | Secrets Manager `tf-1/platform/webhook` | manual | alert ingestor |
...
|---|---|---|---|
| Audit log | S3 bucket `tf-1-audit` | `tf-1-audit-cmk` (CMK) | Object Lock 90 ngày |
...
- Key policy: only allow `tf-1-*-role` access, deny everyone else.
