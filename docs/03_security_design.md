...
Caption: public traffic chỉ tới Demo App/Public API qua ALB. AI Engine không public. Metrics/logs nằm ở Prometheus/Loki/CloudWatch, không dump raw vào SQS. Toàn bộ hạ tầng (sandbox/staging/prod) deploy tại single-region `us-east-1` đúng theo TF1 client brief — không có region deviation.
...
| NAT Gateway | Đang bật (`enable_nat_gateway=true`): worker/pod ở private subnet egress qua NAT để gọi Slack/Jira và pull image. Sandbox 1 NAT (2 AZ), Prod 1 NAT/AZ (3 AZ). VPC endpoints xử lý phần lớn AWS API để giảm lưu lượng NAT. |
...
| Logs | Loki / CloudWatch Logs | Redaction, retention 7 ngày (`log_retention_days=7`); không log token/header nhạy cảm. |
...
Retention: logs 7 ngày (`log_retention_days=7`); S3 evidence và DynamoDB incident state 30-90 ngày; security test outputs giữ tới final defense.
...
| SQ-04 | ~~Bedrock có bật thật không?~~ **RESOLVED (per contracts): AI engine compute-first, Bedrock synthesis OPTIONAL** (`enable_bedrock_endpoint=false` ở sandbox; AI team bật `ENABLE_AGENTCORE_LLM`/`BEDROCK_MODEL_ID` khi dùng). Cost cap khi bật xem `05_cost` (Nova Micro). | AIO-01 + CDO-05 | Done |
...
| SQ-07 | ~~Region cuối cùng `us-east-1` hay `ap-southeast-1`?~~ **RESOLVED: `us-east-1`** cho mọi env, khớp client brief. | CDO-05 | Done |
